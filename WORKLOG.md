...
- MERGED to main (merge commit 5356343, --no-ff), branch + worktree cleaned up, pushed to origin/main.

## 2026-07-15 (multi-monitor bugfix)
- BUG (Giuseppe, with external monitor attached): recording indicator invisible everywhere when dictating with the external LG connected. Isolated in worktree `.worktrees/fix-notch-multimonitor`, branch `fix/notch-multimonitor` (commit 7ee3f4f).
- Root cause (traced through DynamicNotchKit source, not guessed): the LG is his Main Display and has no notch. `.auto` resolves to `.floating` on non-notch screens (`effectiveStyle`: `screen.hasNotch ? .notch : .floating`), and `_compact()` on a floating style does `await hide()` and returns, plus `NotchlessView` only ever renders `expandedContent` in `.expanded` state (compact content parked off-screen at `-windowHeight`). So our `compact()` call showed nothing on any non-notch screen. Single-screen testing on 2026-07-14 masked it because the built-in has a notch.
- Fix: `RecordingIndicator` now targets the focused screen (`NSScreen.main`) and branches: `compact(on:)` if the screen has a notch (the hug), else `expand(on:)` (visible floating pill). Notch detection replicates the library's own `auxiliaryTopLeftArea/auxiliaryTopRightArea` check so our compact/expand choice can never disagree with the style the library resolves. Cached single `.auto` DynamicNotch instance is kept (its expand/compact re-init the window on the passed screen when `window.screen != screen`), so it follows the user across monitors. Listening->Thinking still cross-fades in place via the reactive State object (the library's `guard state != .compact/.expanded` early-returns; the label updates reactively regardless).
- 37/37 checks green (UI-only change, suite doesn't cover RecordingIndicator). Built + installed via bundle.sh --install, stable cert, permissions survived.
- Live-verified by Giuseppe: works. (External-monitor case was the reported break; built-in notch case unchanged.)
- Design choice: indicator follows keyboard focus (appears on the screen you're working on). If he ever wants it pinned to one screen or mirrored on all, that's a small change.
- PENDING: merge fix/notch-multimonitor -> main + push. (Done: merged as be611b5, on main.)

## 2026-07-20 (env fix + notch waveform & history-on-hover)
- ENV (not a code bug): Giuseppe reported "Flusso non becca piu l'italiano". Root-caused with method: `--selftest-asr fixtures/it.wav` transcribes Italian perfectly ("Ciao, questo e un test di dettatura per flusso..."), so the ASR/model are fine. The garble came from the MICROPHONE (Shokz OpenDots in headset/HFP mode = radiolina audio); switching back to the MacBook mic fixed it ("adesso funziona"). See memory [[flusso_ollama_autostart]].
- ENV: Ollama was OFF (no cleanup). Cause: homebrew binary, not a service, no autostart; Mac rebooted 16/7 22:49; WiseMe (which used to keep it alive) uninstalled 12/7. Fixed permanently: `brew services start ollama` (agent homebrew.mxcl.ollama.plist), model qwen2.5:7b still present; full pipeline re-verified on fixtures/it.wav (cleaned Italian, no translation).
- FEATURE (his idea): notch live waveform + tap-Fn-to-peek history. Isolated in worktree `.worktrees/feature-notch-history`, branch `feature/notch-history`. Spec `docs/superpowers/specs/2026-07-18-notch-history-design.md`, plan `docs/superpowers/plans/2026-07-20-notch-history.md`.
- What it does: Listening now shows a live voice waveform (5 bars driven by mic RMS). Quick-tap Fn (no speech) leaves a subtle "storico" hint ~2s; hover it (or hover the indicator any time it's up) to open the last 5 dictations, click a row to copy it to the clipboard ("Copiato"), paste with Cmd+V. Normal hold+speak dictation is unchanged, no added delay.
- How: pure maths in FlussoCore (`AudioLevel` RMS/normalize, `HistoryDisplay` snippet/relative-time) so FlussoChecks can test them (+8 checks, 45 total green). `AudioRecorder.onLevel` emits a throttled 0..1 level from the existing tap. `RecordingIndicator` rewritten: waveform / peek / history views, `DynamicNotch(hoverBehavior: [.keepVisible,.increaseShadow])`, forwards `$isHovering` to AppState. `AppState` pumps level, snapshots `history.recent(5)`, on hover cancels the pending recording and shows history, copies on click; a 2s linger task backs the tap-to-peek.
- Commits: 17bc6d3 (helpers), b9ad71e (recorder), 362978a (indicator), 5df63f6 (appstate). Build (release) + install via bundle.sh --install OK, stable cert.
- ACCEPTED by Giuseppe on real hardware ("Sembra funzionare", "<3"). Merged feature/notch-history -> main (--no-ff), worktree + branch cleaned up, pushed to origin/main.
- Watch item for later if it ever bugs him: hover stability across the compact->expanded transition. If the list ever closes too eagerly while hovering, add a ~150ms debounce on `hovering == false` before acting on it in `RecordingIndicator`/`AppState.handleIndicatorHover`.
