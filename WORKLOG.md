...
- PENDING: Giuseppe's live acceptance of the waveform + hover on real hardware (built-in notch AND external LG). Hover stability across the compact->expanded transition is the one thing to watch; add a ~150ms debounce on hovering==false if it closes too eagerly. Then merge feature/notch-history -> main + push.
